...
//  EvoZhuk
//
//  Created by Dima Senchik on 7/11/19.
...
extension UIDevice {
    ...
    func getFreeSize() -> Double {
        ...
        if let dictionary = try? FileManager.default.attributesOfFileSystem(forPath: paths.last!) {
            if let freeSize = dictionary[FileAttributeKey.systemFreeSize] as? NSNumber {
                return Double(freeSize.int64Value/1000000000)
            }
        } else {
    ...
    func getTotalSize() -> Double {
        ...
        if let dictionary = try? FileManager.default.attributesOfFileSystem(forPath: paths.last!) {
            if let totalSize = dictionary[FileAttributeKey.systemSize] as? NSNumber {
                return Double(totalSize.int64Value/1000000000) + 1
            }
        } else {
            print("Error Obtaining System Memory Info")
        }
    ...
    func getRamSize() -> Double {
        ...
        let formattedSize = Double(size)/1000000000
        return round(formattedSize)
    }
